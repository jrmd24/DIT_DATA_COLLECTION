...
def scrap_equipments(num_pages) -> pd.DataFrame:
	...
	for p in range(1,num_pages+1):
		...
		info_class = ["listing-item__header"
		...
		for link in links:
			...
			try:
				...
				soup_article = bs(res_article.text, 'html.parser') #stocker le code html dans un objet bs

				obj = {}
				for container_type, name, tag, elem_class in zip(info_container_type,info_names, info_tags, info_class):
					try:
							current_info = None
							current_info_text = ""
							if '_' in container_type:
									current_info = soup_article.find(tag.split('_')[0],class_=elem_class).find_all(tag.split('_')[1])
									current_info_text = ",".join([x.text.strip() for x in current_info])
							elif not ("class" in container_type):
									current_info = soup_article.find(tag,class_=elem_class)[container_type]
	...
	for p in range(1, num_pages+1):
		...
		for container in containers :
			try:
				...
				marque = inf[1].text
				annee = inf[2].text
				prix = container.find('span', class_ = 'listing-card__price__value 1').text.strip().replace('\u202f', '').replace(' F Cfa', '')
				adresse = container.find('div', class_ ='listing-card__header__location').text.replace('\n', '').split(',')
				quartier = adresse[0]
				region = adresse[1]
				lien_image = container.find('img', class_ = 'listing-card__image__resource vh-img')['src']
...
def scrap_voitures(num_pages) -> pd.DataFrame:
	...
	for p in range(1,num_pages+1):
		...
		for voiture in voitures:
				try :
						Etat=voiture.find('span', class_='listing-card__header__tags__item--condition').text
						Marque= voiture.find('span', class_='listing-card__header__tags__item--make').text
						Annee=voiture.find('span', class_='listing-card__header__tags__item--buildyear').text
						Boite_vitesse=voiture.find('span', class_='listing-card__header__tags__item--transmission').text
						Adresse= voiture.find('div',class_='listing-card__header__location').text.strip().replace('\n', '')
						Prix= voiture.find('span', class_ = 'listing-card__price__value').text.strip()
						Image_lien=voiture.find('img','listing-card__image__resource vh-img')['src']
