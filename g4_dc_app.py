...
page_options = ('Télécharger données collectées', 'Collecter données', 'Remplir formulaire', 'Voir tableaux de bord')
...
Vehicles = pd.read_csv('TP_Group4_Blanche.csv', sep=";" )
Motocycles = pd.read_csv('web_scraper_url2_G4.csv', sep=";" )
equipements = pd.read_csv('Equipement_piece_1_Cleaned.csv', sep=";" )
...
def treat_price_column(price_txt):
    ...
    if isinstance(price_txt, int):
        return price_txt
    price = price_txt.lower().replace('cfa','').replace('f','').replace(' ','').replace('\u202f','')
    return int(price)
...
def load_charts():
    ...
    mean_prix_auto_par_annee_par_marque = grouped_marque_auto_par_annee["prix_nbr"].mean()
    ...
    cleaned_motos = Motocycles[Motocycles["annee"].notnull() & Motocycles["prix"].notnull() ]
...
if page == page_options[0]:
    ...
    """, width=900, height=1200, scrolling=False)
